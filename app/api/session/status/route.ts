...
export async function POST(request: Request) {
  ...
  if (!userId || !validation.valid) {
    ...
    const response = NextResponse.json({ authenticated: false }, { status: 401, headers: noStoreHeaders() });
    response.cookies.set(SESSION_COOKIE_NAME, "", getExpiredActivityCookieOptions());
    return response;
  }

  return NextResponse.json(
    { authenticated: true, userId, issuedAt: validation.payload.issuedAt, lastActivity: validation.payload.lastActivity },
